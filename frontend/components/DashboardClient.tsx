...
import {
  ...
  Activity, Building2, Droplets, Zap, Loader2,
} from 'lucide-react';
...
const AUTHORITIES = ['Smart City Lucknow', 'PWD_UP', 'NHAI', 'LDA', 'LMC', 'Jal Nigam', 'UPLCL', 'UPSIDA'];
  ...
  useEffect(() => {
    ...
    patchedStyle().then(style => {
      ...
      map.on('load', () => {
        ...
        map.addLayer({
          id: 'heat', type: 'heatmap', source: 'pts', maxzoom: 10,
          paint: {
            'heatmap-weight': 1,
        ...
        map.addLayer({
          id: 'circles', type: 'circle', source: 'pts', minzoom: 7,
          paint: {
        ...
        map.on('mouseenter', 'circles', (e) => {
          ...
          const severityColor = p.status === 'Active' || p.status === 'ACTIVE' ? '#10b981' : '#f59e0b';
...
export default function Dashboard() {
  ...
  // Fetch projects from API — map GeoJSON features → ProjectNode[]
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        if (projRes.ok) {
          const geojson = await projRes.json();
          const nodes: ProjectNode[] = (geojson.features || []).map((f: { properties: Record<string, string | number>; geometry: { coordinates: number[] } }) => ({
            id: f.properties.id as string,
            title: f.properties.title as string,
            division: (f.properties.division as string) || "Lucknow Division",
            lng: f.geometry.coordinates[0],
            lat: f.geometry.coordinates[1],
            authority: f.properties.project_authority as string,
            status: f.properties.status === 'ACTIVE' ? 'Active' : 'Delayed',
            budget: Number(f.properties.budget) || 0,
            type: f.properties.permitType as string,
          }));
          setAllProjects(nodes);
        }

        if (reportsRes.ok) {
          const reports = await reportsRes.json();
          setComplaints(reports.map((r: Record<string, string | number>) => ({
            id: r.id,
            description: r.description,
            category: r.category || 'Safety Hazard',
  ...
  const data = useMemo(() => division === "All Divisions" ? allProjects : allProjects.filter(d => d.division === division), [division, allProjects]);
  ...
  const severityCount = { High: filteredComplaints.filter(c => c.severity === 'High').length, Medium: filteredComplaints.filter(c => c.severity === 'Medium').length, Low: filteredComplaints.filter(c => c.severity === 'Low').length };
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-white font-bold tracking-widest uppercase text-sm animate-pulse">Loading Dashboard Data...</p>
        </div>
      </div>
    );
  }
      ...
      <main className="flex-1 flex flex-col xl:flex-row overflow-hidden">
        ...
        <div className="w-full xl:w-[480px] flex flex-col gap-0 overflow-y-auto border-r border-slate-800 bg-slate-900">
          ...
          <div className="p-5 grid grid-cols-2 gap-3 border-b border-slate-800">
            ...
            <KpiCard label="Delayed" value={delayed} icon={<AlertTriangle size={16} />} color="text-amber-400" sub={`${total > 0 ? Math.round(delayed / total * 100) : 0}% of total`} />
            <KpiCard label="Incidents" value={filteredComplaints.length} icon={<AlertCircle size={16} />} color="text-rose-400" sub={`${severityCount.High} high severity`} />
          ...
          <div className="flex border-b border-slate-800">
            {(['incidents', 'analytics'] as const).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                ...
                {tab === 'incidents' ? `🚨 Incidents (${filteredComplaints.length})` : '📊 Analytics'}
              </button>
            ))}
          </div>
              ...
              <div className="flex flex-col gap-2">
                {filteredComplaints.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-slate-600">
                    ...
                    <p className="font-bold text-sm">No incidents reported yet</p>
                  </div>
                  ...
                  <div key={c.id} className="p-3.5 rounded-xl bg-slate-800/50 hover:bg-slate-800 border border-slate-700/50 hover:border-slate-600 transition-all">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-[10px] font-black text-slate-500 tracking-widest uppercase">{c.id.substring(0, 8)}</span>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-slate-600 font-medium">{c.created_at?.substring(0, 10) || ''}</span>
                      </div>
                    </div>
                    <p className="text-sm font-bold text-slate-200 mb-1.5">{c.description}</p>
                    <p className="text-[10px] text-slate-500 font-semibold uppercase tracking-wide">{c.category}</p>
                  </div>
                ))}
              </div>
            ) : (
              <div className="flex flex-col gap-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
                  ...
                  <div className="flex gap-2 items-center mb-2">
                    <div className="h-2 rounded-full bg-blue-500" style={{ width: `${total > 0 ? Math.round(active / total * 100) : 0}%`, flex: 'none' }} />
                    <div className="h-2 rounded-full bg-amber-500" style={{ width: `${total > 0 ? Math.round(delayed / total * 100) : 0}%`, flex: 'none' }} />
                  </div>
