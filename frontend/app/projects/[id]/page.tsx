...
import { Share2, FileText, CheckCircle2, AlertTriangle, PhoneCall, ExternalLink, Info, UserCircle2, Clock, ScrollText, MapPin, ArrowLeft, Loader2 } from 'lucide-react';
...
const LOGO_MAP: Record<string, string> = {
    ...
    'Smart City Lucknow': '/logos/lda.svg',
    'Jal Nigam': '/logos/lmc.svg',
    'UPLCL': '/logos/pwd.svg',
    'UPSIDA': '/logos/nhai.svg',
};
        ...
        return (
            <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex items-center justify-center">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-slate-600 dark:text-slate-300 font-bold tracking-widest uppercase text-sm animate-pulse">{t.loading}</p>
                </div>
            </div>
        );
    }
    ...
    if (error || !project) {
        return (
            <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-slate-600 dark:text-slate-300 font-bold text-lg">{t.notFound}</p>
                <Link href="/" className="px-6 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-500 transition">{t.back}</Link>
            </div>
        );
    }

    const rtiRef = `RTI/UP/2026/${project.id.substring(0, 3).toUpperCase()}`;
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 font-sans p-6 pb-20">
            ...
            <div className="max-w-4xl mx-auto flex justify-between items-center mb-8">
                <Link href="/" className="px-4 py-2 bg-white dark:bg-slate-800 shadow-sm border border-slate-200 dark:border-slate-700 rounded-xl font-bold hover:bg-slate-50 dark:hover:bg-slate-700 transition flex items-center gap-2">
                ...
                <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-sm border border-slate-200 dark:border-slate-700 relative overflow-hidden">
                    <div className="flex justify-between items-start">
                        <div>
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <span className={`flex items-center gap-1.5 px-3 py-1.5 font-bold uppercase tracking-widest text-[11px] rounded-full ${project.status === 'ACTIVE'
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-400 font-bold flex flex-wrap items-center gap-4 uppercase tracking-wider">
                                ...
                                <span className="flex items-center gap-1"><MapPin size={16} /> {project.district} · {project.division}</span>
                            </p>
                        </div>
                    ...
                    <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-700/50">
                        <div className="flex justify-between items-end mb-3">
                            ...
                            <span className="text-3xl font-black text-blue-600 dark:text-blue-400">{project.completion_percent}%</span>
                        </div>
                        <div className="w-full h-5 bg-slate-100 dark:bg-slate-700/80 rounded-full overflow-hidden shadow-inner">
                            <div className="h-full bg-gradient-to-r from-blue-500 to-indigo-600 rounded-full transition-all duration-1000" style={{ width: `${project.completion_percent}%` }}></div>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col gap-8">
                        <div className="flex items-center gap-5">
                            ...
                            <img src={LOGO_MAP[project.project_authority] || '/logos/lda.svg'} alt={project.project_authority} className="w-20 h-20 rounded-2xl object-contain bg-slate-50 p-2 border border-slate-100 dark:border-slate-700 shadow-sm" />
                            <div>
                                ...
                                <p className="font-black text-xl text-slate-800 dark:text-white flex items-center gap-2">
                                    {project.project_authority} {project.is_verified && <CheckCircle2 size={18} className="text-blue-500" />}
                                </p>
                            </div>
                        </div>
                    ...
                    {/* Timeline, Budget & Reports */}
                    <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col gap-6">
                        <div className="grid grid-cols-2 gap-6">
                            <div>
                                <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2">{t.startDate}</p>
                                <p className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2 text-lg"><Clock size={16} className="text-blue-500" />{formatDate(project.startDate)}</p>
                            </div>
                            <div>
                                <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2">{t.endDate}</p>
                                <p className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2 text-lg"><Clock size={16} className="text-indigo-500" />{formatDate(project.endDate)}</p>
                            </div>
                        </div>
                        <div className="p-4 bg-emerald-50 dark:bg-emerald-900/10 rounded-2xl border border-emerald-100 dark:border-emerald-900/30">
                            <p className="text-[11px] font-black text-emerald-600 uppercase tracking-widest mb-1">{t.budget}</p>
                            <p className="font-black text-3xl text-emerald-700 dark:text-emerald-400">₹{project.budget} Cr</p>
                        </div>
                        <div className="flex justify-between items-center p-6 bg-rose-50 dark:bg-rose-900/10 rounded-2xl border border-rose-100 dark:border-rose-900/30">
                            <div>
                                ...
                                <p className="font-black text-3xl text-rose-600 dark:text-rose-400">{project.report_count}</p>
                            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="relative flex items-center justify-between gap-4 bg-emerald-900/20 p-5 rounded-2xl border border-emerald-500/20">
                            <div>
                                ...
                                <p className="font-mono font-bold text-emerald-400 text-sm tracking-wide">{rtiRef}</p>
                            </div>
                            <button
