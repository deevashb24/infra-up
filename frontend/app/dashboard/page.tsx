...
// Generate fake data across divisions
const MOCK_DATA = Array.from({ length: 400 }).map((_, i) => {
    const isLucknow = i < 80;
    const division = isLucknow ? "Lucknow Division" : DIVISIONS[Math.floor(Math.random() * (DIVISIONS.length - 1)) + 1];
    
    // Approximate centers
    const lng = division === "Lucknow Division" ? 80.8 + Math.random() * 0.3 : 77.5 + Math.random() * 6.5;
    const lat = division === "Lucknow Division" ? 26.7 + Math.random() * 0.3 : 24.8 + Math.random() * 5.0;
    
    const authority = (i % 4 === 0 && division === "Lucknow Division") ? 'Smart City Lucknow' : 'PWD_UP';
    const status = Math.random() > 0.7 ? "Delayed" : "Active";
    ...
    return {
        ...
        authority,
        status,
        budget: Math.floor(Math.random() * 100) + 10 // Crores
    };
});
